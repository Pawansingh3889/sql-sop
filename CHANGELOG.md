...
### Added
- **DBT005 `select-star-in-mart`** - warns on `SELECT *` in models under `models/marts/` (or configured marts path). Marts are published datasets; `SELECT *` silently adopts upstream schema changes. ([#80](https://github.com/Pawansingh3889/sql-sop/pull/80))
