...
### Added
- **config:** accept `.sql-sop.yml` and `.sql-sop.yaml` as project config filenames alongside `.sql-guard.yml` and `.sql-guard.yaml`. If both `.sql-sop` and `.sql-guard` are found in the same folder, `.sql-sop` is used and a warning is printed. If only `.sql-guard` is found, a deprecation notice is printed. ([#101](https://github.com/Pawansingh3889/sql-sop/issues/101))
